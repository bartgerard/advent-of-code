...
public record ScratchCard(
    ...
    static int totalNumberOfScratchCardsWon(final Collection<ScratchCard> scratchcards) {
        ...
        final Map<Integer, Integer> nbCardsById = new HashMap<>();

        for (final ScratchCard reversedCard : reversedCards) {
            final int nbMatchingNumbers = reversedCard.matchingNumbers().size();

            final int totNumberOfScratchCardsWonForId = IntStream.rangeClosed(1, nbMatchingNumbers)
                    .map(i -> reversedCard.id() + i)
                    .map(id -> nbCardsById.getOrDefault(id, 0))
                    .sum();

            nbCardsById.put(reversedCard.id(), 1 + totNumberOfScratchCardsWonForId);
        }

        return nbCardsById.values()
                .stream()
